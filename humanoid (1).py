...
# Choose observer poles (5x faster than controller)
eig_cl = np.linalg.eigvals(Acl)
observer_poles = 5 * eig_cl
...
def observer_system(t, z):
    ...
    # Observer
    dx_hat = A @ x_hat + Bu @ u.flatten() + L @ (y - y_hat)
...
sol_obs = solve_ivp(observer_system, [0, 5], z0, t_eval=t)
...
axs[0].plot(t, x[:,0]*180/np.pi, label='Actual θ1')
axs[0].plot(t, x_hat[:,0]*180/np.pi, '--', label='Estimated θ1')
axs[0].set_ylabel('θ1 (deg)')
axs[0].set_title('Comparison of Actual and Estimated Joint Angles')
axs[0].legend()

axs[1].plot(t, x[:,1]*180/np.pi, label='Actual θ2')
axs[1].plot(t, x_hat[:,1]*180/np.pi, '--', label='Estimated θ2')
axs[1].set_ylabel('θ2 (deg)')
axs[1].set_xlabel('Time (s)')
axs[1].legend()

plt.tight_layout()
plt.savefig("observer_states.png", bbox_inches='tight')

error = x - x_hat

fig, axs = plt.subplots(2, 1, sharex=True)

axs[0].plot(t, error[:,0]*180/np.pi)
axs[0].set_ylabel('Error θ1 (deg)')
axs[0].set_title('Estimation Error Convergence')

axs[1].plot(t, error[:,1]*180/np.pi)
axs[1].set_ylabel('Error θ2 (deg)')
axs[1].set_xlabel('Time (s)')

plt.tight_layout()
plt.savefig("observer_error.png", bbox_inches='tight')

fig, axs = plt.subplots(2, 1, sharex=True)

axs[0].plot(t, x[:,2], label=r'Actual $\dot{\theta}_1$')
axs[0].plot(t, x_hat[:,2], '--', label=r'Estimated $\dot{\theta}_1$')
axs[0].set_ylabel('rad/s')
axs[0].set_title('Comparison of Actual and Estimated Angular Velocities')
axs[0].legend()

axs[1].plot(t, x[:,3], label=r'Actual $\dot{\theta}_2$')
axs[1].plot(t, x_hat[:,3], '--', label=r'Estimated $\dot{\theta}_2$')
axs[1].set_ylabel('rad/s')
axs[1].set_xlabel('Time (s)')
axs[1].legend()

plt.tight_layout()
plt.savefig("observer_velocity.png", bbox_inches='tight')
